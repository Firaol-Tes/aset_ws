...
% Author: Firaol Tesfaye
%
% TODO before submission:
%   [x] Author list + emails confirmed (A. Natsheh first)
%   [x] Reviewer revisions applied: tool table (II), scenario table (IV),
%       failure-mode table (VIII), params table (V), LLM-config table (III),
%       compute-cost table (VII), Wilson CIs + preliminary framing, HSV/YOLO
%       clarified, limitations strengthened, recent refs added
%   [x] Figures 1-4 in place; all cross-refs resolve (no "Fig ??" on 2-pass)
%   [ ] Compile with pdflatex TWICE (resolves refs); needs tikz, pgfplots,
%       IEEEtran + the figures/ folder
% =========================================================================
...
same tool services with a fixed, perception-blind sequence and no recovery.
Single-object pick-and-deliver is reported complete in 13/16 trials, most
of them involving an autonomous recovery from a failed first grasp; because
outcomes were self-reported rather than checked against the simulator, four
of those runs end with a failed or timed-out place, bounding the true rate
...
\begin{enumerate}
    ...
    \item a comparison against a hardcoded baseline sharing the identical
    tool services, which isolates decision-making from stack capability but
...
VoxPoser~\cite{voxposer}, form a parallel line; recent surveys catalogue
...
baseline sharing the identical action interface
...
loops for up to a maximum number of turns (default 30). At each turn the
...
repeats the sequence for a second object. The baseline is thus fast,
...
of model calls and failed tool calls. The simulated warehouse and robot
...
step for the baseline---and a \emph{failure} if any step aborts, the run
exceeds the turn limit (Table~\ref{tab:llmconfig}) or the per-tool 200\,s
timeout, or the planner calls \texttt{task\_complete(success=false)}. This
criterion is self-reported and was not checked against the simulator;
...
details the four scenarios, their initial conditions, and stopping criteria.
...
\midrule
S1 & LLM & ``take the red cube to dispatch''; red cube on table & 5 & red cube in dispatch zone & abort, turn limit, or 200\,s tool timeout \\
S2 & LLM & ``bring the red cube to dispatch''; all three cubes on table & 7 & the \emph{red} cube in dispatch zone & abort, turn limit, or 200\,s tool timeout \\
S4 & LLM & as S1, cube displaced $\approx$18\,cm 5\,s after departure & 4 & red cube in dispatch zone & abort, turn limit, or 200\,s tool timeout \\
...
under test: a later commit changed the drive speeds (above) and corrected
...
per trial to a shared CSV. The LLM results below are computed from the
per-task logs, and the scenario each run belongs to is recovered from the
...
and since the mid-task disturbance is applied only by the runner, they are
nominal-condition runs by construction. The baseline results are from
...
deliveries in 143--249\,s (mean 194\,s); the failure was an abort at
navigation start. Being perception-blind and branch-free, it is roughly
...
controller, the LLM times from the planner, and the same baseline task run
through the experiment runner takes 256--507\,s wall-clock including reset
overhead. And because the baseline is non-reactive \emph{by construction},
...
\emph{determinism versus adaptability} on a shared action interface, and
...
\begin{axis}[
  ...
  \node[above, align=center, font=\scriptsize, text=inkprimary] at (axis cs:1,81)
        {\textbf{81\%}\textsuperscript{$\ast$}\\[-1pt]{\scriptsize\color{inksecondary}13/16}};
  \node[above, align=center, font=\scriptsize, text=inkprimary] at (axis cs:2,83)
        {\textbf{83\%}\textsuperscript{$\dagger$}\\[-1pt]{\scriptsize\color{inksecondary}5/6}};
...
A companion technical dossier documents every version, coordinate, tool,
and result with provenance.
